...
function playCurrentScene(reset) {
  if (!chart.value) {
    return;
  }
...
watch(paused, (val) => {
  ...
  val ? chart.value.getZr().animation.pause() : chart.value.getZr().animation.resume();
...
onMounted(() => {
  // Init chart
  chart.value = init(containerRef.value, undefined, {
    useDirtyRect: true,
  });
...
useEventListener(containerRef.value, onContainerResize);
