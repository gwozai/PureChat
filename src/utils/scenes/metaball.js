...
graphic.registerShape("metaball", MetaballPath);
...
function getCircleShape(api, dataIndex) {
  const width = api.getWidth();
  const height = api.getHeight();
  const viewSize = Math.sqrt(width * width + height * height) / 2;

  const ring = api.value(0, dataIndex);
  const angle = api.value(1, dataIndex);
    ...
    series: [
      {
        ...
        renderItem(params, api) {
          ...
          const fromIdx = data[params.dataIndex][3];

          const fromShape = fromIdx != null ? getCircleShape(api, fromIdx) : null;
...
export default new Scene({
  ...
  duration: [100, ...dataAll.slice(1).map((a) => 1500), ...SVG_SHAPES.map((a) => 500)],
  dark: true,
  // background: '#225369',
  background: "#373940",
});
